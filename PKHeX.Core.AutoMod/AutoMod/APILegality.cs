...
namespace PKHeX.Core.AutoMod
{
    ...
    public static class APILegality
    {
        ...
        )
        {
            ...
            foreach (var enc in encounters)
            {
                ...
                if (raw.IsEgg) // PGF events are sometimes eggs. Force hatch them before proceeding
                    raw.HandleEggEncounters(enc, tr, tb);

                raw.PreSetPIDIV(enc, set, criteria, tb);
        ...
        internal static GameVersion[] FilteredGameList(
            PKM template,
            GameVersion destVer,
            bool batchEdit,
            IBattleTemplate set,
        ...
        )
        {
            if (batchEdit)
            {
                if (set is RegenTemplate { Regen.VersionFilters: { Count: not 0 } x })
                {
                    if (TryGetSingleVersion(x, out var single))
                        return single;
                }
            }
            ...
            var gamelist =
                (!nativeOnly && AllowHOMETransferGeneration)
                    ? [.. versionlist.OrderByDescending(c => c.GetGeneration())]
                    : GetPairedVersions(destVer, versionlist);

            if (PrioritizeGame && !nativeOnly)
            {
                gamelist = PrioritizeGameVersion == GameVersion.Any
                    ? PrioritizeVersion(gamelist, destVer.GetIsland())
                    : PrioritizeVersion(gamelist, PrioritizeGameVersion);
            }
        ...
        private static bool TryGetSingleVersion(
            IReadOnlyList<StringInstruction> filters,
            [NotNullWhen(true)] out GameVersion[]? gamelist
        )
        {
            gamelist = null;
            foreach (var filter in filters)
            {
                ...
                gamelist = filter.Comparer switch
                {
                    ...
                    InstructionComparer.IsNotEqual
                        => GameUtil.GameVersions.Where(z => !result.Contains(z)).ToArray(),
                    InstructionComparer.IsGreaterThan
                        => GameUtil.GameVersions.Where(z => result.Any(g => z > g)).ToArray(),
                    InstructionComparer.IsGreaterThanOrEqual
                        => GameUtil.GameVersions.Where(z => result.Any(g => z >= g)).ToArray(),
                    InstructionComparer.IsLessThan
                        => GameUtil.GameVersions.Where(z => result.Any(g => z < g)).ToArray(),
                    InstructionComparer.IsLessThanOrEqual
                        => GameUtil.GameVersions.Where(z => result.Any(g => z <= g)).ToArray(),
                    _ => result,
                };
        ...
        )
        {
            ...
            if (set.Species == (ushort)Species.Pikachu)
            {
                switch (enc.Generation)
                {
                    ...
                    case >= 7
                        when set.Form != (enc is EncounterInvalid or EncounterEgg ? 0 : enc.Form):
                        return false;
                }
            }
        ...
        private static void ApplySetDetails(
            PKM pk,
            IBattleTemplate set,
        ...
        )
        {
            ...
            pk.SetPINGA(set, pidiv.Type, set.HiddenPowerType, enc, tb);
            pk.SetSpeciesLevel(set, Form, enc, language, tb);
            pk.SetDateLocks(enc);
            pk.SetHeldItem(set, tb);

            // Actions that do not affect set legality
            pk.SetHandlerandMemory(handler, enc, tb);
            pk.SetFriendship(enc, tb);
            pk.SetRecordFlags(set.Moves, tb);

            // Legality Fixing
            pk.SetMovesEVs(set, enc, tb);
            pk.SetCorrectMetLevel(tb);
            pk.SetGVs(tb);
            pk.SetHyperTrainingFlags(set, enc, tb); // Hypertrain
            pk.SetEncryptionConstant(enc, tb);
            pk.SetShinyBoolean(set.Shiny, enc, regen.Extra.ShinyType, tb);
            pk.FixGender(set, tb);
        ...
        private static void ValidateGender(PKM pk)
        {
            ...
            if (!genderValid)
            {
                if (pk.Format == 4 && pk.Species == (ushort)Species.Shedinja) // Shedinja glitch
                {
                    ...
                    pk.Format > 5
                    && pk.Species is (ushort)Species.Marill or (ushort)Species.Azumarill
                )
                {
                    var gv = pk.PID & 0xFF;
            ...
            else
            {
                ...
                {
                    pk.Gender = EntityGender.GetFromPID(
                        new LegalInfo(pk, []).EncounterMatch.Species,
        ...
        private static void SetHyperTrainingFlags(
            this PKM pk,
            IBattleTemplate set,
            IEncounterable enc,
            ITracebackHandler tb
        ...
        /// <param name="tb"></param>
        private static void HandleEggEncounters(
            this PKM pk,
            IEncounterable enc,
            ITrainerInfo tr,
            ITracebackHandler tb
        ...
        )
        {
            ...
            var changeec =
                hascurry != null
                && string.Equals(hascurry, "true", StringComparison.OrdinalIgnoreCase)
                && AllowBatchCommands;
        ...
        )
        {
            ...
            {
                ivsetfor = nameof(IStaticCorrelation8b);
                var flawless = 0;
            ...
            {
                ivsetfor = nameof(EncounterEgg) + " (BDSP)";
                pk.IVs = set.IVs;
                Shiny shiny;
                if (set is RegenTemplate r)
                    shiny = r.Regen.Extra.ShinyType;
        ...
        public static void FindWildPIDIV8(
            PK8 pk,
            Shiny shiny,
            int flawless = 0,
            uint? fixedseed = null
        )
        {
            ...
            var editnecessary =
                (shiny == Shiny.AlwaysSquare && pk.ShinyXor != 0)
        ...
        public static void FindEggPIDIV8b(PKM pk, Shiny shiny, byte? gender)
        {
            ...
            while (true)
            {
                ...
                if (
                    ratio != PersonalInfo.RatioMagicMale
                    && ratio != PersonalInfo.RatioMagicFemale
                    && ratio != PersonalInfo.RatioMagicGenderless
                )
                {
        ...
        private static bool IsMatchCriteria9(
            PK9 pk,
        ...
        )
        {
            ...
            {
                if (!FormInfo.IsFormChangeable(pk.Species, pk.Form, template.Form, EntityContext.Gen9, pk.Context))
                    return false;
            }
            ...
            return pk.Generation switch
            {
                ...
                4
                    => info.EncounterMatch switch
                    {
                        ...
                        EncounterStatic4 s
                            => (s.Shiny == Shiny.Always ? PIDType.ChainShiny : PIDType.Method_1),
        ...
        private static void FixEdgeCases(this PKM pk, IEncounterable enc, ITracebackHandler tb)
        {
            ...
            if (
                pk is IGeoTrack
                && pk.Species
                    is (int)Species.Vivillon
                        or (int)Species.Spewpa
                        or (int)Species.Scatterbug
            )
            {
        ...
        /// </summary>
        ///
        public static EncounterCriteria SetSpecialCriteria(
            EncounterCriteria criteria,
            IEncounterable enc,
            IBattleTemplate set
        )
        ...
        )
        {
            AsyncLegalizationResult GetLegal()
            {
                try
                {
                    if (!EnableDevMode && ALMVersion.GetIsMismatch())
                        return new(template, LegalizationResult.VersionMismatch, null);
                    ...
                    return new AsyncLegalizationResult(res, s, traceback);
                }
                catch (MissingMethodException)
                {
                    return new AsyncLegalizationResult(
                        template,
            ...
            return first ?? new AsyncLegalizationResult(template, LegalizationResult.Timeout, null);
        }
            ...
            ITracebackHandler? traceback)
        {
            ...
            public readonly ITracebackHandler? Traceback = traceback;
        }
        ...
        private static GameVersion[] GetPairedVersions(
            GameVersion version,
            IEnumerable<GameVersion> versionlist
        )
        {
